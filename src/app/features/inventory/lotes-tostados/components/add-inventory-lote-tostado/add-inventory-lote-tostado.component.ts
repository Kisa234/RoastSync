...
import { InventarioLoteTostadoService } from '../../service/inventario-lote-tostado.service';
...
@Component({
  selector: 'add-inventory-lote-tostado',
  standalone: true,
  imports: [CommonModule, FormsModule, LucideAngularModule],
  templateUrl: './add-inventory-lote-tostado.component.html',
})
export class AddInventoryLoteTostadoComponent implements OnInit {
  @Input() loteTostadoId: string = '';
  ...
  lote: LoteTostadoConInventario | null = null;
  almacenes: Almacen[] = [];
  ...
  private loadData(): void {
    if (!this.loteTostadoId) {
      this.uiService.alert('error', 'Error', 'No se recibió el lote tostado.');
      this.onCancel();
      return;
    }
    ...
    forkJoin({
      lotes: this.loteTostadoService.getLotesTostadosConInventario(),
      almacenes: this.almacenService.getAlmacenesActivos(),
    }).subscribe({
      next: ({ lotes, almacenes }) => {
        this.lote = (lotes ?? []).find(
          (l: LoteTostadoConInventario) => l.id_lote_tostado === this.loteTostadoId
        ) || null;
        ...
        if (!this.lote) {
          this.uiService.alert('error', 'Error', 'No se encontró el lote tostado.');
          this.onCancel();
        }
      },
      error: () => {
  ...
  getCantidadTotalLote(): number {
    return Number(this.lote?.peso ?? 0) || 0;
  }
  ...
  getCantidadRestante(): number {
    return this.getCantidadTotalLote() - this.getCantidadAsignada();
  }

  getIdsAlmacenesExistentes(): string[] {
    return (this.lote?.inventarioLotesTostados ?? []).map(
      (i: InventarioLoteTostado) => i.id_almacen
    );
  }
  ...
  getAlmacenesDisponibles(index: number): Almacen[] {
    ...
    return this.almacenes.filter((a: Almacen) => {
      const usadoEnOtraFila = this.rows.some(
        (row, i) => i !== index && row.id_almacen === a.id_almacen
      );
  ...
  validate(): boolean {
    ...
    for (let i = 0; i < this.rows.length; i++) {
      ...
      if (
        ...
        row.cantidad_kg === undefined ||
        Number(row.cantidad_kg) < 0
      ) {
        this.uiService.alert(
          'warning',
          'Validación',
          `La cantidad de la fila ${i + 1} debe ser mayor o igual a 0.`
        );
        return false;
      }
      ...
      if (this.isAlmacenYaUsado(row.id_almacen, i)) {
        this.uiService.alert(
          'warning',
          'Validación',
          `El almacén de la fila ${i + 1} ya fue asignado.`
        );
        return false;
      }
    }

    const totalLote = this.getCantidadTotalLote();
    const totalAsignado = this.getCantidadAsignada();

    if (totalAsignado !== totalLote) {
      this.uiService.alert(
        'warning',
        'Cantidad incorrecta',
        `La suma asignada (${totalAsignado}) debe ser igual al total del lote (${totalLote}).`
      );
      return false;
    }
  ...
  save(): void {
    ...
    if (!this.lote?.id_lote_tostado) return;

    this.saving = true;

    const requests = this.rows.map((row: InventoryRow) =>
      this.inventarioLoteTostadoService.create({
        id_lote_tostado: this.lote!.id_lote_tostado,
    ...
    forkJoin(requests).subscribe({
      next: () => {
        ...
        this.uiService.alert(
          'success',
          'Inventario asignado',
          'El inventario del lote tostado fue asignado correctamente.'
        );
        this.create.emit();
      },
      error: (error) => {
        ...
        const backendMessage =
          error?.error?.error ||
          error?.error?.message ||
          'No se pudo asignar el inventario del lote tostado.';
