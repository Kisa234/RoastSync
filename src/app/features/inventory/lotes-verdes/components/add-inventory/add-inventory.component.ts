...
export class AddInventoryComponent implements OnInit {
  ...
  lote: LoteVerdeConInventario | null = null;
  almacenes: Almacen[] = [];
  ...
  private loadData(): void {
    ...
    forkJoin({
      lotes: this.loteService.getLotesVerdesConInventario(),
      almacenes: this.almacenService.getAlmacenesActivos(),
    }).subscribe({
      next: ({ lotes, almacenes }) => {
        this.lote = (lotes ?? []).find(l => l.id_lote === this.loteId) || null;
        this.almacenes = almacenes ?? [];
        this.loading = false;
        this.loadingAlmacenes = false;

        if (!this.lote) {
          this.uiService.alert('error', 'Error', 'No se encontró el lote.');
          this.onCancel();
        }
      },
      error: () => {
  ...
  getIdsAlmacenesExistentes(): string[] {
    return (this.lote?.inventarioLotes ?? []).map(i => i.id_almacen);
  }
  ...
  save(): void {
    ...
    const requests = this.rows.map(row =>
      this.inventarioLoteService.create({
    ...
    forkJoin(requests).subscribe({
      next: () => {
        this.saving = false;
        this.uiService.alert('success', 'Inventario asignado', 'El inventario del lote fue asignado correctamente.');
        this.create.emit();
      },
      error: (error) => {
        this.saving = false;
        ...
        const backendMessage =
          error?.error?.error ||
          error?.error?.message ||
          'No se pudo asignar el inventario del lote.';
